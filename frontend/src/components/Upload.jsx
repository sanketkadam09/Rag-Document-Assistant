...
const BASE_URL = process.env.REACT_APP_BACKEND_URL || "http://13.234.168.225:5000";
...
function Upload({ selectedDoc, setSelectedDoc }) {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      console.error("Failed to fetch documents:", err);
      alert("Failed to fetch documents. Check your backend connection.");
    }
  };
  ...
  const handleUpload = async (event) => {
    ...
    try {
      ...
      console.error("Upload failed:", err);
      alert("Upload failed. Make sure backend is running and CORS is enabled.");
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleDelete = async (doc) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/documents/${doc}`);
      alert(`${doc} deleted successfully`);
      if (selectedDoc === doc) setSelectedDoc(null);
      fetchDocuments();
    } catch (err) {
      console.error("Delete failed:", err);
      alert("Delete failed. Make sure backend is running and CORS is enabled.");
    }
  };
